title: open-agent Auto Dreaming：异步记忆整合机制
...
tags:
  ...
  - AI
  - Memory
description: 介绍 open-agent 内置的 Auto Dreaming 机制，涵盖触发调度、三阶段 sweep、评分模型、持久化结构与当前限制。
---

Auto Dreaming 是 [open-agent](https://github.com/tutar/open-agent) 内置的异步记忆整合机制，灵感来自认知科学中的睡眠记忆巩固理论：Agent 在每轮对话结束后，在后台以三阶段 sweep 将高价值短期记忆自动晋升为持久化的 `MemoryRecord`，不阻塞主流程。
